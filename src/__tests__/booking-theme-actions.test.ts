...
import { requireShopId as mockRequireShopId, getAuthSession as mockGetAuthSession, getShopIdBySlug as mockGetShopIdBySlug, createServiceRoleClient as mockCreateServiceRole } from "@/lib/dashboard/auth-server";
...
describe("upsertBookingTheme", () => {
  it("upserts theme data and revalidates paths", async () => {
    ...
    expect(mockRevalidatePath).toHaveBeenCalledWith("/dashboard/mi-local/business");
...
describe("uploadBookingLogo", () => {
  ...
  it("uploads and stores logo path", async () => {
    const pngFile = new File(["fake-png"], "logo.png", { type: "image/png" });
